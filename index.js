...
function handleSearchItemSubmit() {
  $('#js-shopping-search-form').submit(function(event) {
    event.preventDefault();
    console.log('`handleSeachItemSubmit` ran');
    const searchItemName = $('.js-shopping-search-entry').val();
    searchFilter(searchItemName);
    renderShoppingList();
  });
}
...
function editItemName()
{
  $('.js-shopping-list').on('click', '.js-item-edit', event => {
    // get the index of the item in STORE
    const item_id = getItemIdFromElement(event.currentTarget);
    // delete the item
    let indexOfItem = STORE.items.findIndex(item => item.id === item_id)

    let edited = prompt("Enter new list item name",STORE.items[indexOfItem].name);
    
    if(edited === '')
    {
      edited = STORE.items[indexOfItem].name; 
    }
    
    STORE.items[indexOfItem].name = edited;
